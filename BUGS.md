...
- `rknpu_devfreq.h` emits real _declarations_ (not inline stubs) because `CONFIG_PM_DEVFREQ` is defined.
...
**Root cause:** `docker buildx rm` removes the builder _container_ but does **not** delete the
...
1. **BuildKit registry cache replay.** The bldr build pipeline caches compilation
   ...
   to `ccflags-y` in `Kbuild` changes a _file_ copied during prepare, but if that
...
2. **GCC `-U` flag ordering vs `-include` files.** GCC processes all `-D`/`-U`
   command-line flags _before_ processing any `-include` files. If `autoconf.h`
...
(same magic, same ABI, same machine type, same e*flags=0x0). It is NOT glibc 2.36
per se (Ubuntu 24.04 with glibc 2.39 works fine). Best hypothesis: Debian 12's
gcc/binutils toolchain emits an AArch64-specific ELF note (`GNU_PROPERTY_AARCH64*
...
The YOLOv5s C API result is _worse_ than Python because the longer sustained run
...
in the same `RUN` layer of the _final_ multi-stage image. The layer-diff algorithm
...
_Step 1:_ widen the DMA mask to 40-bit before the allocation. The RK3588 SoC and its ARM
...
_Step 2:_ use `dma_alloc_noncontiguous()` (which previously failed under Rev 1 _because_
...
committed to DRAM. `regcfg_amount` is written into the task struct _immediately_ before
...
- Rev 10 (`flush_dcache_page` after `alloc_page`): Removes stale _zero cache lines_
...
**Bug 55 REGRESSION — still broken on the versions this fork actually builds
(canary 2026-07-19):** Rev 31 above is marked "actual fix", but it was only ever
verified on rknpu.ko 0.9.8 / kernel 6.18.22-talos. On the versions this fork now ships
— **rknpu.ko 0.9.17.4, librknnrt.so 2.3.2, Talos v1.13.6 / kernel 6.18.38-talos** — a
real Turing RK1 canary shows the driver loads and `init_runtime()` succeeds, but **every
inference still fails**:

```
E RKNN: failed to submit! ... int status: 0
```

dmesg shows the upstream's own later debug instrumentation:

```
BUG55 r55c poll[0]: ist=0x0
BUG55 r55c poll[1]: ist=0x0
...
BUG55 r55c poll[9]: ist=0x0
```

`ist` (interrupt status) stays `0x0` across the entire poll loop — the NPU **job-done
interrupt never asserts**, so submitted jobs never complete (this is a step past the
Rev 31 sticky-fault-bit theory: there `INT_RAW_STATUS=0xc0000000`; here it is `0`, i.e.
the completion IRQ simply never fires). The `r55c` label is upstream's own iteration
marker — the last upstream commits on this bug (2026-05-05, "post-OP*EN diagnostic
polling") were still \_debugging* it, not fixing it. **Conclusion: Bug 55 is NOT resolved
on the shipped stack. RKNN inference is non-functional on this fork's v1.13.6 build.**
The downstream consumer (Immich ML in `taloskubecluster`) has been kept on the CPU
backend; do not wire it to `rockchip.com/npu` until the job-done IRQ is fixed upstream
(bits 30+31 of `INT_MASK=0xc0000000`). Deep RK3588 hw/IRQ work, low ROI — NPU path
shelved.

**Observed on:** rknpu.ko 0.9.17.4, librknnrt.so 2.3.2, Talos v1.13.6 /
kernel 6.18.38-talos, Turing RK1 (RK3588). Fault-bit variant (Rev 31) was on 6.18.22.

---

_Add new bugs above this line, most recent first._
